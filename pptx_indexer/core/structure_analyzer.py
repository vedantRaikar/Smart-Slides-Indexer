...
class StructureAnalyzer:
    ...
    def _extract_topic(self, title: str) -> str:
        ...
        title = re.sub(r"^(part|chapter|section|lesson|unit|module)\s*\d*\s*[-:]*\s*", "", title, flags=re.IGNORECASE)
        return title.strip()
    ...
    def detect_repeated_themes(self, slides: List[SlideNode]) -> Dict[str, List[str]]:
        ...
        repeated = {
            theme: slides for theme, slides in keyword_slides.items()
            if len(slides) >= 3
        }
    ...
    def _get_stop_words() -> Set[str]:
        ...
        return {
            "the", "and", "or", "a", "an", "to", "in", "is", "be", "at",
            "this", "that", "with", "for", "from", "by", "on", "can",
            "will", "was", "are", "have", "has", "had", "do", "does",
            "did", "as", "of", "if", "about", "what", "which", "who",
            "when", "how", "why", "more", "most", "no", "not", "also",
        }
